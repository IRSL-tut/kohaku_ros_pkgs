...
import numpy as np
import math
from kohaku_ros_driver.msg import (
    EmptyAction,
    EmptyFeedback,
    EmptyResult)
...
class KohakuActionServerNode(object):
    def __init__(self):
        rospy.init_node('kohaku_action_server_node')
        ...
        self._arm_dof = 6
        self._hand_dof = 2
        self._arm_mask = [0, 0, 0, 0, 0, 0, 1, 1]
        self._gripper_mask = [1, 1, 1, 1, 1, 1, 0, 0]
        self._gripper_stall_current = 0.7
        self._gripper_command_tolerance = 0.05
        self._close_hand_angle = -0.2
        self._finger_link_length = 0.06
    ...
    def set_joint_trajectory_srv_client(self, target_angles, goal_time, mask,
    ...
    def _calc_hand_angles_from_gripper_position(self, position):
        try:
            base_length = self._finger_base_length
            link_length = self._finger_link_length
            theta = math.asin((position - base_length) / (2.0 * link_length))
    ...
    def _get_relocated_target_angles(self, trajectory_joint_names, trajectory_points):
        # 関節角度の並べ替え
        target_angles_list = []
        for pt in trajectory_points:
            target_angles = []
            for jn in self._joint_names:
                if jn in trajectory_joint_names:
                    jn_index = trajectory_joint_names.index(jn)
                    target_angles.append(pt.positions[jn_index])
            target_angles_list.append(target_angles)

        return target_angles_list
    ...
    def update_arm_trajectory_feedback(self, start_time):
        ...
        if self._joint_angles is not None:
            self._arm_trajectory_as_feedback.actual.positions = self._joint_angles[0:self._arm_dof]
        if self._joint_speeds is not None:
            self._arm_trajectory_as_feedback.actual.velocities = self._joint_speeds[0:self._arm_dof]
        if self._joint_accs is not None:
            self._arm_trajectory_as_feedback.actual.accelerations = self._joint_accs[0:self._arm_dof]
        if self._joint_torques is not None:
            self._arm_trajectory_as_feedback.actual.effort = self._joint_torques[0:self._arm_dof]

        # publish feedback
        self._arm_trajectory_as.publish_feedback(self._arm_trajectory_as_feedback)

    def get_gripper_status(self, target_pos):
        hand_angles = self._joint_angles[self._arm_dof:]
        hand_torques = self._joint_torques[self._arm_dof:]
        current_position = self._get_gripper_position(hand_angles)
        current_effort = self._get_gripper_effort(hand_torques)
        hand_currents = self._joint_currents[self._arm_dof:]
        if any([abs(x) > self._gripper_stall_current for x in hand_currents]):
            stalled = True
        else:
            stalled = False
    ...
    def execute_arm_trajectory_waypoint(self, goal):
        ...
        # 関節角度の並べ替え
        target_angles_list = self._get_relocated_target_angles(goal.trajectory.joint_names,
                                                               thinned_points)

        for target_angles in target_angles_list:
            ...
            self.set_joint_trajectory_srv_client(target_angles,
                                                 goal_time_sec,
                                                 self._arm_mask,
    ...
    def execute_arm_trajectory_direct(self, goal):
        ...
        target_angles_list = self._get_relocated_target_angles(goal.joint_names,
                                                               goal.trajectory.points)
        pt = goal.trajectory.points[-1]
        start_time = rospy.Time.now()
        target_time = start_time + pt.time_from_start
        target_angles = target_angles_list[-1]
        target_angles.extend([0.0, 0.0])
    ...
    def execute_gripper_command_cb(self, goal):
        ...
        if hand_angles is not None:
            ...
            target_angles = [0] * (self._arm_dof + self._hand_dof)
        ...
        if self.check_reached_max_effort(goal.command.max_effort):
            ...
            self.set_joint_trajectory_srv_client(adapt_angles,
